...
namespace DeadLords.Shooter.Controllers
{
    ...
    public class InputController : BaseController
    {
        ...
        public void Update()
        {
            ...
            {
                Main.Instance.GetObjectManager.Weapons[indexWeapon].IsVisible = false;
                Main.Instance.GetObjectManager.Ammo[indexWeapon].IsVisible = false;
            ...
            else if (Input.GetAxis("Mouse ScrollWheel") < 0)
            {
                ...
                if (indexWeapon == Main.Instance.GetObjectManager.Weapons.Length - 1)
                    indexWeapon = 0;
                else
                    indexWeapon++;
            ...
            else if (Input.GetAxis("Mouse ScrollWheel") > 0)
            {
                ...
                if (indexWeapon == 0)
                    indexWeapon = Main.Instance.GetObjectManager.Weapons.Length - 1;
                else
                    indexWeapon--;
            }   //Предыдущее
            else if (_isSelectedWeapon) return;     //Если оружие выбрано - дальше не идем

            Main.Instance.GetObjectManager.Weapons[indexWeapon].IsVisible = true;
            Main.Instance.GetObjectManager.Ammo[indexWeapon].IsVisible = true;
